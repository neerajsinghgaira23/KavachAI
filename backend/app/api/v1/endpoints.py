...
async def scan_payload(payload: ScanRequest):
    """
    Main threat inspection pipeline:
    1. Sanitizes input content & normalizes whitespace
    2. Extracts candidate URLs, emails, and root domains
    3. Runs pre-compiled regex heuristics for advance-fee fraud, rental scams, and off-platform redirection
    4. Evaluates domain RDAP age & typosquatting brand-jacking with 3.5s timeout and offline fallback
    5. Evaluates sender email reputation & recruiter impersonation
    6. Calculates dynamic 0-100% Scam Threat Index, builds contextual safe replies & remediation advice
    """
    ...
    # 1. Evaluate regex patterns on text content
    flags_raw = evaluate_regex_rules(sanitized_body)

    # 2. Evaluate sender email
    has_mail_flag = False
    if sender_email:
        mail_eval = analyze_sender_email(sender_email, sanitized_body)
        if mail_eval:
            has_mail_flag = True
            flags_raw.append({
    ...
    # 3. Evaluate domain intelligence via RDAP
    domain_obj: Optional[DomainInfo] = None
    domain_name = extract_domain(target_url) if target_url else ""

    if domain_name:
        rdap_res = await lookup_domain_rdap(domain_name, timeout=settings.RDAP_TIMEOUT_SECONDS)
        if rdap_res:
            domain_obj = DomainInfo(**rdap_res)
            # Add domain flag if newly registered or high-risk TLD
    ...
    # 4. Calculate Threat Score & Remediations
...
async def detect_ai_generated_text(req: AIDetectRequest):
    ...
    burstiness_score = round(std_dev_wc / (mean_wc + 0.001), 2)  # Low burstiness (<0.35) is characteristic of AI
    ...
    for s in sentences:
        ...
        # Calculate approximate pseudo-perplexity based on marker density and uniform length
        marker_hits = []
        ...
        # Perplexity estimation: lower is more predictable (synthetic)
        # Uniform length close to mean + marker hits reduces perplexity score
    ...
    # Base formula combining burstiness (inverse), marker density, and synthetic sentence ratio
    raw_ai_prob = (
