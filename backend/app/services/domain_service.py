...
def compute_levenshtein_ratio(s1: str, s2: str) -> float:
    """
    Calculates string similarity ratio between two strings (0.0 to 1.0).
    """
    if s1 == s2:
        return 1.0
...
async def lookup_domain_rdap(domain: str, timeout: float = 3.5) -> Dict[str, Any]:
    """
    Queries public RDAP bootstrap service for registration information.
    Includes offline mock heuristics for resilient demos.
    """
    if not domain:
        return {}
    ...
    # Standard public RDAP query
    try:
        base_url = settings.RDAP_BASE_URL.rstrip("/")
        url = f"{base_url}/{domain}"
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            resp = await client.get(url, headers={"User-Agent": "KavachAI-Threat-Inspector/1.4"})
...
async def analyze_domain(domain_str: str) -> Tuple[DomainInfo, int]:
    """
    Async function analyzing domain:
    - Queries RDAP with 3.5s timeout.
    - Computes domain age and risk scores.
    - Returns (DomainInfo model, domain_score).
    - Score points: Age < 30 days (+35 pts), Age < 90 days (+20 pts), Suspicious TLD (+15 pts), Typosquatting (+35 pts).
    """
